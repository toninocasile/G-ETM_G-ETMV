...
%
% In this version there are additional terms in the model to take care of
% trial-by-trial variability of the spike rates
%
...
% USAGE :
%
% function [beta_Trials, beta_Global, beta_History, dev, beta_Trials_pVals, beta_Global_pVals, beta_History_pVals] = ...
%    fitGLM_G_ETMV(SpikesData_in, globalRegressor, historyRegressor, targetNeuron, nBinsHistory, triggerNeuron)
%
...
dim2 = globalRegressor.nBins + nNeurons*nBinsHistory + nTrials;
dim1 = (lenTrial_samples-nBinsHistory*historyRegressor.binDuration_samples)*nTrials;

Xneuron = zeros(dim1+2, dim2);
Yneuron = zeros(dim1+2, 1);

% now start filling in the matrices X and Y
currRowInd = 1;
for currTrialInd = 1:nTrials
		trialVars = zeros(1, nTrials);
		trialVars(currTrialInd) = 1;
    for currTimeInd = totalHistory_samples+1:lenTrial_samples
        ...
        Xneuron(currRowInd, :) = [trialVars, parmsGlobal, parmsHistory(:)'];
...
% regularizers for the fitting process to converge
Xneuron(dim1+1, 1:nTrials) = 1;
Yneuron(dim1+1) = 0;
Xneuron(dim1+2, 1:nTrials) = 1;
Yneuron(dim1+2) = 1;
...
% now we fit a glm model to our data!
...
beta_Trials = par_est(1:nTrials);
beta_Trials_pVals = par_est_pVals(1:nTrials);

beta_Global = par_est(nTrials+1 : nTrials+globalRegressor.nBins);
beta_Global_pVals = par_est_pVals(nTrials+1 : nTrials+globalRegressor.nBins);
...
TMP = reshape(par_est(nTrials + globalRegressor.nBins+1:end), nBinsHistory, nNeurons)';
TMP1 = reshape(par_est_pVals(nTrials + globalRegressor.nBins+1:end), nBinsHistory, nNeurons)';
